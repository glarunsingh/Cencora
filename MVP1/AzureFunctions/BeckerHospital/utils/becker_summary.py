...
from BeckerHospital.utils.load_model import model
from BeckerHospital.utils.becker_schema import BeckerSchema
from BeckerHospital.utils.prompt_config import prompt_config
...
async def get_summ_sent_key(raw_text, url, client_name, k_list):
    ...
    try:
        ...
        logger.error(f"Failed to generate response  for {url} from LLM.Line No: {exc_tb.tb_lineno}  Error: {str(e)}",
                     stack_info=True)
