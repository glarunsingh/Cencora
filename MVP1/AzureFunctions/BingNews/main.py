...
@bp_bingnews.timer_trigger(schedule=cron_time, arg_name="myTimer", run_on_startup=False,
                           use_monitor=False)
def bing_news_scrapping(myTimer: func.TimerRequest) -> None:
    """
    ...
    try:
        ...
        client_name = ["All"]
        minDate = (datetime.now().date() - timedelta(days=2)).strftime(
        ...
        if client_name[0] == "All":
            client_list = bing_news_db.query_client_list(department_name=department_name)
        else:
            client_list = []
