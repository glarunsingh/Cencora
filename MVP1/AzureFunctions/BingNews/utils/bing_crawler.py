...
import os
import logging
from BingNews.utils import logger

from dotenv import load_dotenv
import sys
from BingNews.utils.database import Bing_News_DB

_ = load_dotenv("./BingNews/config/config.env")
logger = logger.create_log(name="bing_news", level=logging.INFO)

BING_NEWS_Plugin_URL = os.environ['BING_NEWS_PlUGIN_URL']
SCRAPE_PLUGIN_URL = os.environ['SCRAPE_PLUGIN_URL']
...
def bing_news_crawler(client_name, search_term, minDate, location="US"):
    """
    ...
    try:
        ...
        response = requests.post(bing_url, json=payload)
...
def bing_news_prompt():
    ...
    to generate the concise summary for given article content according to given client. Also identify the sentiments from the given content. You also need to extract matching keywords from the content
    based on a list of keywords you are provided. Also identify the content is breaking news and identify the content is about the
    ...
    -----
        ...
        keyword list: ''' {keywords_list} ''' and
        client:'''{client_name}'''
    -----
    ---------------
    ...
    2. Identify the overall sentiment of the article scrapped content, it should be either Positive, Negative, or 
        Neutral.
    3. List of matching keywords from the article scrapped based on a list of keywords provided.
    4. Is Breaking news true, or false
    5. Is News is about the given client, client should be from Healthcare industry only true, or false
...
class BingCrawl:
    ...
    def __init__(self, source_name, department_name, minDate, location, store_db):
        ...
        for attempt in range(retries):
            try:
                ...
                prompt = PromptTemplate(
                    ...
                    input_variables=["raw_content", "keywords_list", "client_name"],
                    partial_variables={"format_instructions": output_parser.get_format_instructions()}
                )
                chain = prompt | model | output_parser
                output_dict = chain.invoke(
                    {"raw_content": html_content, "keywords_list": self.keyword_list, "client_name": client_name})
    ...
    def news_summary(self, url_data, search_term):
        ...
        try:
            ...
            # Make a POST request
            response = requests.post(SCRAPE_PLUGIN_URL, json=payload)
            search_results = response.json()
