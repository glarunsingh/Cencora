...
def create_dataframe(data):
    ...
    df['news_date'] = df['news_date'].apply(convert_utc_date)
    df = df.rename(columns={'news_date': 'Date', "news_title": "Title", "news_url": "URL", "news_summary": "Summary",
                            "sentiment": "Sentiment"})
    df.index += 1
    return df


def create_excel(df, f_path):
    workbook = Workbook()
...
def delete_file_after_delay(f_path):
    try:
        delay=120
        time.sleep(delay)
