...
class HIMSSDBOPS:
    ...
    def query_items(self, input_date: list):
        ...
        try:
            ...
            query_text = (f"SELECT c.news_date, c.news_url FROM c WHERE c.source_name = 'HIMSS' AND (") +query_join + f")"
            query_params = [
    ...
    def query_and_sort_items_by_date(self, source_name, limit):
            ...
            try:
                container = self.database.get_container_client(self.NEWS_CONTAINER_NAME)
                query_text = f"SELECT c.news_date, c.news_url FROM c WHERE c.source_name = @source_name ORDER BY c.news_date DESC OFFSET 0 LIMIT @limit"
                query_params = [
