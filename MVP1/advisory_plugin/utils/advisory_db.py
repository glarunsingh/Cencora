...
class DBOPS:
    ...
    def __init__(self,container_name):
        ...
        self.DATABASE_ID = os.getenv('COSMOS_DATABASE')
...
class AdvisoryDBOPS(DBOPS):
    ...
    """
    def __init__(self):
        DBOPS.__init__(self,'COSMOS_CONTAINER')
    ...
    def query_excel_items(self,input_date,source_name):
        '''
        ...
        try:
            ...
            print(f'\nQuerying from {self.CONTAINER_ID} for given {source_name}\n')
            items = list(container.query_items(
                query=f"SELECT r.news_title,r.news_url,r.news_summary,r.news_date,r.sentiment \
                    FROM r WHERE r.source_name=@source_name and ({query_multidate})",
                parameters=[
                    { "name":"@source_name", "value": source_name }
                ],
                enable_cross_partition_query=True
...
class KeywordDBOPs(DBOPS):
    ...
    def __init__(self):
        ...
        '''
        DBOPS.__init__(self,'KEYWORD_CONTAINER')
    ...
    def query_keyword_list(self,department_name):
        ...
        """
        try:
            logger.info(f'\nQuerying keyword list from {self.CONTAINER_ID} for {department_name}\n')
