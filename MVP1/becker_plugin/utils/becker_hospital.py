...
from utils.becker_summary import get_summary,get_summ_sent_key
from utils.becker_db import BeckerDBOPS,KeywordDBOPs
...
class BeckerCrawl:
    ...
    async def becker_crawling(self,client_name,duration,scrape_existing=False,
                              persist=True,department_name="Health",source_name="becker review hospital"):
        """
                ...
                for article in articles:
                    ...
                    if time_element is not None:
                        ...
                        if not scrape_existing and article_link in url_list:
                            ...
                            continue
                        # Get the link text
                        article_text = post_info.text.replace("\t","").replace("\n","")

                        ##Get the news content
                        article_news = await self.fetch_news_content(article_link)
    ...
    async def batch_scrape_all_client(self):
        """
        function for bulk run for all the clients
        """
        ...
        for client in clients:
            print("Running For client {}".format(client))
            article_info = await self.becker_crawling(client,duration='all',
                                                      scrape_existing=False,
                                                      persist=True,
                                                    department_name="Health",
                                                    source_name="becker review hospital")
            
            if article_info==None:
                print("Failed For client {}".format(client))
                self.failure_cnt=self.failure_cnt+1
            else:
                self.all_articles_info.extend(article_info)
        return self.all_articles_info