...
from utils.llm_scrapper import extract_article_list_llm
...
async def get_articles_list_dc(year: int, month: int, headers=url_headers(),use_llm=False):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if use_llm:
                non_empty_articles = await extract_article_list_llm(soup)
    ...
    # except (requests.exceptions.RequestException, requests.exceptions.ConnectionError, TimeoutError, ssl.SSLError,
    #         HTTPError, URLError, ProxyError, InvalidURL) as e:
    #     #failure_cnt = failure_cnt + 1
    #     exc_type, exc_obj, exc_tb = sys.exc_info()
    #     logger.error(f"Url Request Error - Line No: {exc_tb.tb_lineno}  Error: {str(e)}", stack_info=True)
